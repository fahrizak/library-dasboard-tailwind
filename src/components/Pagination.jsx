...
const Pagination = ({totalPage, onPageChange, limit, currentPage, siblings, dataLength}) => {
    ...
    return (
        <div className="table__footer lg:flex flex-row justify-between items-center px-4">
        <div className="tampilkan max-lg:hidden">
            Menampilkan {((currentPage - 1) * limit) + 1} - 
            {` ${ totalPage !== currentPage ?  currentPage * limit : dataLength} `} data dari 
            {" " + dataLength} data
        </div>

        <ul className="pagination flex text-sm gap-x-1 my-6 justify-center items-center">
