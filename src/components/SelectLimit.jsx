...
const SelectLimit = (props) => {
    ...
    return (
        ...
        <div onClick={toggleClass} className={` relative px-2 py-1 border border-gray-300 rounded-full w-24 inline-block cursor-pointer`}>
            <div className=" w-full flex justify-between ml-1 h-6 items-center text-sm">
                {props.limit} Data
                <i className="ri-arrow-down-s-line absolute bottom-[4px] right-2 text-[1.3rem]"></i>
            </div>

            <div className={`${showMenu}  absolute flex flex-col border border-gray-300 bg-white w-24 left-0 top-10 rounded-lg px-0.5 text-xl`}>
